...
__Exercise__. With our informative prior and our non-informative prior, we will now collect some data to include in our Bayesian analysis. After you collect data, derive the posterior distribution of $p$ using both the non-informative prior and the informative prior. Then, adjust the code above to produce a plot of your two posterior distributions.
...
alpha2 <- 87
k2 <- 32.3
gamma_density2 <- dgamma(lambda_grid, shape = alpha2, rate = k2)

gamma_plot <- tibble(lambda_grid, gamma_density, gamma_density2)
ggplot(data = gamma_plot, aes(x = lambda_grid, y = gamma_density)) +
  geom_line() +
  geom_line(aes(y = gamma_density2), colour = "darkred") +
  theme_minimal()
```
